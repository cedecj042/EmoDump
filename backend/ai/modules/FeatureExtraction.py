...
interjections_df = pd.read_csv(os.path.join(settings.DATA_DIR,'data/interjections.csv'))
negations_df = pd.read_csv(os.path.join(settings.DATA_DIR,'data/negations.csv'))
amplifiers_df = pd.read_csv(os.path.join(settings.DATA_DIR,'data/amplifiers.csv'))
emotionlex_df = pd.read_csv(os.path.join(settings.DATA_DIR,'data/emolex_words.csv'))
emointensity_df = pd.read_csv(os.path.join(settings.DATA_DIR,'data/emolex_intensity.csv'))
...
def get_emolex(text):
    filepath = os.path.join(settings.DATA_DIR,"data/NRC-Emotion-Lexicon-Wordlevel-v0.92.txt")
...
def load_lst():
    path = os.path.join(settings.DATA_DIR,"data/frequency_distributions.pkl")
...
def get_average_score(clean_tweet):
    ...
    df_single = calculateAverage(df_single)
    return df_single['average_score']
